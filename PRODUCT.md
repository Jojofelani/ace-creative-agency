...
- **Generic dark-SaaS agency template** — near-black + one accent + three
  identical feature cards + an uppercase eyebrow over every section. The default
  we actively fight.
- **Corporate + stock photography** — safe, interchangeable big-agency polish
  with meaningless imagery.
- **Cold Swiss / grid minimalism** — austere, gridded, emotionally flat;
  precise but lifeless, the opposite of "leave feeling more creative."

Not an anti-reference: loud, ambitious visual effects. One bold element is
wanted — the rule is *spend boldness once*, not avoid boldness.

## Design Principles

- **Everything eases.** No linear motion, no snapping. Damping on cursor,
  easing on scroll, momentum via Lenis. Eased motion reads designed and
  expensive; snapping reads like a broken widget.
- **Spend boldness once.** The crystal is the single loud element; every other
...
Target WCAG AA: body contrast ≥4.5:1, large text ≥3:1, visible keyboard focus.
Honor `prefers-reduced-motion` — kill idle and scroll motion, keep a still
crystal. Ship a lighter or static crystal on mobile / low-power devices; a heavy
WebGL scene must not stutter on mid-range Android over slow data. Preloader for
heavy 3D assets, no white flash. The decorative canvas is hidden from assistive
tech; real text and meta/OG tags live in the DOM (canvas content isn't
crawlable).
